...
> [!IMPORTANT]
> 创建容器时取消勾选**非特权容器 (Unprivileged container)**  
> 勾选**非特权容器 (Unprivileged container)**，会导致密码无法生效
...
`IPv4` 选择 `DHCP`，`IPv6` 选哪个都一样，容器都会获得一个无状态 IPv6 地址
...
为 OCI 类型容器添加的 Apparmor profile ，lxc.init.cmd 和 lxc.mount.auto（后两者仅限 OCI 容器使用）
