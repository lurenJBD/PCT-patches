...
> [!IMPORTANT]
> 在脚本修改后，会对部分 LXC 容器产生影响，如：在 Debian LXC 容器内运行 Docker 或挂载 NFS  
...
> 2. 请撤销脚本修改后重启 PVE，重新运行脚本后再次重启 PVE 后，再尝试新建 Redroid LXC 容器  
> 3. 若无法确认是否成功，可[查看部署成功截图](#为-oci-类型容器添加的-apparmor-profile-lxcinitcmd-和-ixcmountauto-参数后两者仅限-oci-容器使用)来确定是否成功
...
> [!Tip]
> 如果想关闭 IPv6，请在 `lxc.init.cmd` 参数中添加 `androidboot.disable_ipv6=1` [仅限 Lineage 模板支持]
